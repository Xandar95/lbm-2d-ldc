...
This code simulates the 2D lid-driven cavity flow problem using the Lattice Boltzmann Method (LBM) with the D2Q9 lattice model, and a Single Relaxation Time (SRT) collision operator. The simulation is performed on a square domain with a moving lid at the top, and no-slip boundary conditions on the other walls.
...
def compute_feq(rho, ux, uy, cx, cy, w, nx, ny):
...
def lbm_step(f, rho, ux, uy, cx, cy, w, omega, nx, ny):
    # compute feq
    feq = compute_feq(rho, ux, uy, cx, cy, w, nx, ny)
    # collision step
    f += -omega * (f - feq)
    # streaming step
...
def compute_macroscopic(f, cx, cy):
    rho = np.sum(f, axis=2)
...
if __name__ == "__main__":
    rho, ux, uy, X, Y, nx, ny, x, y, Lx, Ly, u_lid, Re = run_simulation()
...
plt.plot(data_u[:, 2], data_u[:, 0], 'ro', label='Ghia et al. (1982)')
plt.legend()
...
plt.plot(data_v[:, 0], data_v[:, 2], 'bo', label='Ghia et al. (1982)')
plt.legend()
